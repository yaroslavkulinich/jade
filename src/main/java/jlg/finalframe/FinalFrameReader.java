...
//        logger = LoggerFactory.getLogger("jlg.final-frame-api");
    ...
    public byte[] read(InputStream is) {
        try {
            ...
            System.out.println("(FinalFrameReader::read) Error while reading data from stream. Final frame packet will be discarded.");
            return null;
        }
    }
    ...
    private boolean isFooterValid() {
        for (int i = 0; i < finalFrameFooter.length; i++) {
            ...
            if (unsignedValue != 165) {
                ...
                System.out.println("(FinalFrameReader::read) Dropped final frame packet because an invalid " +
    ...
    private boolean isHeaderValid(int frameLength, int availableBytes) {
        if (availableBytes < frameLength) {
            ...
            nbOfDroppedFinalFramePackets++;
            System.out.println("(FinalFrameReader::read) Dropped final frame packet because its size exceeds the remaining available stream. " +
